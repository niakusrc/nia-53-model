...
# from import_data import load_data, load_prediction_data, min_max_calc # 1차 데이터 임포트
from import_data_v2 import load_data, load_prediction_data, min_max_calc, load_test_data  # 2차 데이터 임포트
import pandas as pd
import warnings
...
class BaseModel():
    ...
    def train(self,region, x_train=None, y_train=None):
        x_train, y_train, x_valid, y_valid, x_test, y_test = load_data(region)
        ...
        for idx in range(self.epoch):
            ...
            eval_loss = self.model.evaluate(x_valid, y_valid, batch_size=self.batch_size, verbose=1)
            print('eval_loss : ',eval_loss)
            patience +=1
            if patience > self.args.patience:
                break
            if best_eval_loss_pretrain > eval_loss[-2]:
                self.save_model(self.model_name+'pretrain_best', save_dir=self.args.save_dir)
                best_eval_loss_pretrain = eval_loss[-2]
                patience = 0
    ...
    def test(self,region, x_test=None, y_test=None, is_save=True):
        x_test, y_test = load_test_data(region)
            # x_train, y_train, x_test, y_test = load_data(region)
        ...
        if is_save:
            ...
            output.save_test_output(pred_inverse, y_inverse,x_test, output_path)
    ...
    def show_result(self):
        df = pd.read_csv('output/test_pred.csv')

    def prediction(self,region, x_test=None, y_test=None, is_save=True):
        if x_test == None and y_test == None:
            x_train, y_train, x_test, y_test = load_prediction_data(region)
        else:
            x_train, y_train = self.x_train, self.y_train
        self.load_model(self.model_name, self.args.save_dir, best=True)
        # min_val_st, max_val_st = get_min_max(load_np_data(self.args.dataset+'x_st_train.npz'), self.scale)
        max_val_st, min_val_st = min_max_calc(region)
        print('*' * 20)
        print('max_val_st, min_val_st : ',max_val_st, min_val_st)
        print('*' * 20)
        # min_val_st=0.0
        # max_val_st = 200.0
        pred_output = self.model.predict(x_test)
        ...
        if is_save:
            ...
            # pred_inverse = scaler(pred_output, self.scale, inv=False, min_value=min_val_st, max_value=max_val_st)
            # y_inverse = scaler(y_test, self.scale, inv=False, min_value=min_val_st, max_value=max_val_st)
